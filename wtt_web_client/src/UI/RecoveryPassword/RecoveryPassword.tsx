...
const RecoveryPassword: React.FC = () =>
{
    ...
    useEffect(() =>
    {
        try
        {
            ...
            const claims = jwtDecode<IJwtInfo>(jwt);
    ...
    const onSubmit = async () =>
    {
        ...
        setPassErrorMessage("");
        let result = await AuthHelper.ResetPassword(password, jwt);
        if (result === 200)
        {
        ...
        {
            setPassErrorMessage(`We was unable to change your password. Error code: HTTP_${result}.`);
        }
    }
        ...
        <CSSTransition in={transState} {...commonTransProp}>
            <span className={cla.authWrapper}>
                ...
                {/* <span className={[cla.authHeader,cla.credentialsLabel].join(' ')}>
                    You are about to change password for the account:
                </span>
                <input
